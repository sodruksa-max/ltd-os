---
type: knowledge-base
updated: 2026-05-11
---

# Thesis Tracker

Active investment theses. Nick's universe walk + Reese's research frame draw from here.

## Active Theses

### T1 — AI Capex Supercycle
**Statement:** Hyperscaler capex (MSFT/AMZN/GOOGL/META) remains elevated → semicon + datacenter = direct beneficiary for 3-5 years.
**Tickers:** NVDA, AMD, AVGO, SMCI, DELL, HPE, MU, MRVL, LRCX, MOD, CBRS (watch)
**Kill condition:** Hyperscaler capex guidance 2 consecutive quarters below prior year → thesis broken.
**Status:** Active — early innings
**Last reviewed:** 2026-05-16
**Notes:** CBRS (Cerebras Systems) added as watch — wafer-scale AI chip, largest US tech IPO 2026 ($5.55B), 86% UAE revenue concentration risk, P/S ~186x; Nick flagged 2026-05-16; full research at vault/20_investment/CBRS-2026-05-16.md

### T2 — Semiconductor Moats
**Statement:** NVDA GPU monopoly in AI training + ASML EUV monopoly = durable pricing power through cycle.
**Tickers:** NVDA, ASML, ARM, AMD, CRDO, AEIS, UCTT, WDC, ONTO
**Kill condition:** Viable NVDA alternatives (Gaudi, MI400, custom ASIC) capture >15% hyperscaler training spend.
**Status:** Active
**Last reviewed:** 2026-05-11

### T3 — Space & Defense AI
**Statement:** Space economy entering commercial ramp — launch, satellite connectivity, defense contracts. AI-native defense software emerging as separate moat.
**Tickers:** RKLB, ASTS, LUNR, KTOS, BBAI, HAWK (watch)
**Kill condition:** RKLB loses government launch contracts; ASTS spectrum/regulatory blocked; BBAI loses DoD renewals.
**Status:** Active — speculative tier
**Last reviewed:** 2026-05-16
**Notes:** HAWK added as watch — space RF intelligence / satellite; Nick flagged 2026-05-16; no research doc yet (run /stock-content HAWK to build)

### T4 — AI Software Monetization
**Statement:** AI infra build-out → software layer begins monetizing: PLTR winning gov + enterprise AI; CRM/SNOW embedding AI into workflow.
**Tickers:** PLTR, CRM, SNOW
**Kill condition:** PLTR loses 2+ large government renewals; AI features fail to lift CRM/SNOW ARR meaningfully.
**Status:** Active
**Last reviewed:** 2026-05-11

### T5 — Quantum Computing
**Statement:** Quantum computing moving from research phase to early commercial deployments — error correction breakthroughs (Google, IBM) are pulling forward timelines. Pure-play names carry high binary risk but asymmetric upside if timelines compress.
**Tickers:** IONQ, RGTI, QBTS, QUBT, IBM
**Kill condition:** No commercially viable error-corrected qubit system demonstrated by 2027; pure-plays fail to win enterprise pilots; IBM quantum roadmap delayed >2 years.
**Status:** Active — pre-commercial, high speculation
**Last reviewed:** 2026-05-11

### T6 — Robotics & Automation
**Statement:** AI + cheaper actuators + better compute = robotics crossing from industrial niche to mass deployment — surgical, warehouse, collaborative, and humanoid robots all scaling simultaneously.
**Tickers:** ISRG, TER, CGNX, ABB, ROK, SYM, SYK, PATH, AVAV
**Kill condition:** Robot unit economics fail to improve (cost per unit stagnant 2+ years); major players miss revenue guidance 2 consecutive quarters; humanoid robot deployments fail to reach commercial scale by 2028.
**Status:** Active — early cycle, multi-year
**Last reviewed:** 2026-05-16

---

## C-List (Watch — not in portfolio yet)

| Ticker | Why watching | Trigger to upgrade |
|---|---|---|
| TSM | Foundry monopoly; geopolitical risk headwind | US fab progress reduces Taiwan risk premium |
| GOOGL | Capex play + search moat under AI pressure | Gemini wins measurable market share back |
| MSFT | AI Capex + Azure cloud + Copilot monetization | Copilot ARR contribution becomes material |
| AMZN | AWS + AI infra + Bedrock | AWS growth reaccelerates above 20% YoY |
| META | AI capex beneficiary + ad monetization | Llama commercial traction demonstrated |

---

## Retired Theses

*(none yet)*

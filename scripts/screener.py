...
Criteria:
  ...
  4. Price      — $5-$500 (exclude penny stocks and ultra-high priced)

Score = RS_vs_SPY * clamp(vol_ratio, 0.5, 3.0)
...
def screen(tickers, bars_data):
    results = []
    fails = []

    # Get SPY bars for benchmark
...
def print_table(results, top=None):
    ...
    print(f"{'#':<3} {'Ticker':<8} {'Price':>8} {'20d Ret':>8} {'RS/SPY':>8} {'VolRatio':>9} {'MA50':>5} {'Score':>8}")
    print("-" * 65)
    for i, r in enumerate(rows, 1):
        ...
        print(
            ...
            f"{r['vol_ratio']:>8.2f}x {ma_flag:>5} {r['score']:>8.4f}"
        )
    print()
    print("Score = RS_vs_SPY * clamp(VolRatio, 0.5, 3.0)")
    print("MA50 Y=above / n=below  |  RS/SPY = outperformance vs S&P 500 (20d)")
...
def main():
    ...
    # Always include benchmark for RS calculation
    ...
    results, fails = screen(tickers, bars_data)

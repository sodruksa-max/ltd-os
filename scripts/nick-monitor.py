#!/usr/bin/env python3
"""
nick-monitor.py — Daily Nick signal update + kill condition alert.

1. Reads tickers dynamically from vault/Knowledge/THESIS_TRACKER.md
2. Fetches RSI/MA20/RS signals for all tickers
3. Updates vault/Knowledge/nick-signals.md (no prices — Nick stays blinded)
4. Parses current holdings from latest weekly-rec.md
5. Checks holdings against alert thresholds:
   RS=WEAK, MA20=BELOW, or RSI=OVERSOLD → alert
6. If any alerts: writes vault/20_investment/nick/alerts/YYYY-MM-DD-alert.md
7. Commits + pushes both files

Replaces: nick-signal.py (superset)
Scheduled: daily Tue-Sat via CCR (covers Mon-Fri US closes)
"""
...
NICK_WEEKLY    = REPO / "vault/20_investment/nick/weekly"
...
# ---------------------------------------------------------------------------
# Parse THESIS_TRACKER.md
# ---------------------------------------------------------------------------

def load_thesis_tickers() -> tuple[list[str], list[str]]:
    """Returns (thesis_tickers, clist_tickers) from THESIS_TRACKER.md."""
...
# ---------------------------------------------------------------------------
# Parse current holdings from latest weekly-rec.md
# ---------------------------------------------------------------------------

def load_current_holdings() -> list[str]:
    """
    Parses the most recent weekly-rec.md for active holdings.
    Matches: '### TICKER — Intact' or '### TICKER — Evolving'
    """
    if not NICK_WEEKLY.exists():
        print("[WARN] Nick weekly dir not found — no holdings to monitor")
        return []

    weekly_files = sorted(NICK_WEEKLY.glob("*_weekly-rec.md"))
    if not weekly_files:
        print("[WARN] No weekly-rec.md files found — no holdings to monitor")
        return []

    latest = weekly_files[-1]
    print(f"Holdings source: {latest.name}")
    text = latest.read_text(encoding="utf-8")

    holdings: list[str] = []
    for match in re.finditer(r"^###\s+([A-Z]{1,5})\s+[—-]+\s+(Intact|Evolving)", text, re.MULTILINE):
...
# ---------------------------------------------------------------------------
# Kill condition alert
# ---------------------------------------------------------------------------

def check_alerts(holdings: list[str], all_rows: list[tuple]) -> list[dict]:
    """Check holdings against kill thresholds. Returns list of triggered alerts."""
    signal_map = {ticker: (r, m, rs) for ticker, r, m, rs in all_rows}
    alerts = []
    for ticker in holdings:
        if ticker not in signal_map:
            continue
        r, m, rs = signal_map[ticker]
        triggers = []
        if r == "OVERSOLD":
            triggers.append(f"RSI={r}")
        if m == "BELOW":
            triggers.append(f"MA20={m}")
        if rs == "WEAK":
            triggers.append(f"RS={rs}")
        if triggers:
            alerts.append({"ticker": ticker, "rsi": r, "ma20": m, "rs": rs, "triggers": triggers})
    return alerts


def write_alert_file(alerts: list[dict], today: date, holdings: list[str]) -> Path:
    ALERTS_DIR.mkdir(parents=True, exist_ok=True)
    alert_path = ALERTS_DIR / f"{today}-alert.md"

    lines = [
        f"# Nick Kill Condition Alert — {today}",
        "",
        f"*Auto-generated by nick-monitor.py | Holdings monitored: {', '.join(holdings)}*",
        "",
        "## Triggered Alerts",
        "",
    ...
    for a in alerts:
        ...
        lines.append(f"| {a['ticker']} | {a['rsi']} | {a['ma20']} | {a['rs']} | **{triggers_str}** |")
...
def git_commit_push(files: list[Path], today: date, alert_count: int) -> None:
    file_args = [str(f) for f in files]
    try:
        subprocess.run(["git", "add"] + file_args, cwd=str(REPO), check=True, capture_output=True)
        msg = f"vault: nick-monitor {today} — {alert_count} alert(s)"
        ...
        if "nothing to commit" in stdout or "nothing to commit" in stderr:
            print("Nothing to commit — signals unchanged")
        else:
            print(f"[WARN] Git operation failed: {stderr[:300]}")
...
def main():
    ...
    print(f"Holdings to monitor ({len(holdings)}): {', '.join(holdings) if holdings else 'none'}")
    ...
    clist_rows: list[tuple] = []
    if clist_tickers:
        print("\n--- C-List tickers ---")
        clist_rows = build_rows(clist_tickers, spy_map)

    all_rows = thesis_rows + clist_rows
    ...
    files_to_commit = [OUTPUT]
    alert_count = 0

    if holdings:
        alerts = check_alerts(holdings, all_rows)
        if alerts:
            alert_path = write_alert_file(alerts, today, holdings)
            files_to_commit.append(alert_path)
            alert_count = len(alerts)
            print(f"\n*** KILL CONDITION ALERT: {alert_count} holding(s) triggered ***")
            for a in alerts:
                print(f"  {a['ticker']}: {' + '.join(a['triggers'])}")
            print(f"Alert saved: {alert_path}")
        else:
            print(f"\nAll {len(holdings)} holdings within normal thresholds — no alerts")
    else:
        print("\n[WARN] No holdings found — skipping kill condition check")
        print("       Run /nick-init or /nick-weekly first")

    # --- Commit + push ---
    git_commit_push(files_to_commit, today, alert_count)

...
#### 4a. Run weekly-snapshot.py (primary — ไม่ใช้ search slot)

```bash
code/python/.venv/Scripts/python scripts/weekly-snapshot.py --week <WEEK>
```

Output ครอบคลุม **ทุก ETF + macro** ที่ต้องการ:
- SPY, QQQ, DIA, IWM, VXX (indexes + VIX proxy) — weekly % + high/low ครบ
- XLE, XLK, XLP, XLU, XLY (sector ETFs) — weekly % + sector ranking
- GLD, TLT, USO, BNO, UUP (gold + bonds + oil + dollar)
- ^VIX weekly high, ^TNX, CL=F, BZ=F, GC=F (macro indicators)
- **Regime verdict** อัตโนมัติ + best/worst sector

ใช้ output ของ script เป็น **Market Data section** โดยตรง (weekly % = fri close vs prev fri close)

**ถ้า script fail** → fallback ใช้ web search แทน (5 queries ตาม list เดิม):
...
#### 4b. Key events + earnings (web search — 1 slot เสมอ)

```
key market events earnings results week of <START_DATE> <YEAR>
```

ใช้ผลนี้สำหรับ **Key Events** และ **Notable Earnings** sections เท่านั้น

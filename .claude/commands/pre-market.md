...
Macro numbers (futures, VIX, yields, oil, gold, DXY, Asia) come from Step 1.5 scripts — only news and sentiment need web searches here.
...
1. `macro-snapshot.py` output (Alpaca ETF proxies + yfinance) — primary for futures, VIX, yields, oil, gold, DXY, Asia
...
While web searches are running, execute both in parallel:

```bash
code/python/.venv/Scripts/python scripts/macro-snapshot.py
code/python/.venv/Scripts/python scripts/sr-levels.py SPY QQQ
```

If today's catalysts or earnings involve specific tickers (e.g. NVDA, AAPL), add them to sr-levels:

```bash
code/python/.venv/Scripts/python scripts/sr-levels.py SPY QQQ NVDA AAPL
```
...
If either script fails, note `[unavailable]` and continue without it.
...
- **เหตุผล 3 ข้อ:**
  ...
  2. [อ้างอิงจาก Catalyst / Asia ใน brief นี้]
...
- **Search cap:** 3 searches max (news + Polymarket + earnings) — macro data comes from scripts, not searches

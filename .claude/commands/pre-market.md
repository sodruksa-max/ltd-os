...
### 1.5 Run S/R levels (parallel with searches)

While web searches are running, execute:

```bash
code/python/.venv/Scripts/python scripts/sr-levels.py SPY QQQ
```

If today's catalysts or earnings involve specific tickers (e.g. NVDA, AAPL), add them:

```bash
code/python/.venv/Scripts/python scripts/sr-levels.py SPY QQQ NVDA AAPL
```

Save the output — it feeds directly into the "Key S/R Levels" section and Trade Setup price levels.
If the script fails (no API key, network error), note `[sr-levels unavailable]` and continue without it.
